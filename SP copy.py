...
while True:
    ...
    # Printing the output (Working)
    def print_lang(text):
        print(text)
        reply = text
        bot.send_message(reply, to)

    # Taking the input from server
    def input_lang():
        # print("Entering input again")
        message = None
        global update_id
        while True:
    ...
    def friends():
        print_lang('How are your friends meeting up with your expectations?'\
              '\n')
        response = input_lang()
        if(predict(response) >=0.4):
            print_lang('Have you broken up with someone recently?\n')
            response = input_lang()
            if(predict(response)>=0.4):
                print_lang(name + ", don't feel sad. Take your time and heal properly,"\
                ...
                print_lang(name + ", don't worry. You may be at a point where similar "\
            ...
            print_lang("Many people tend to expect too much of others, their family, "\
    ...
    def family():
        print_lang(name + ", don't take too much stress. All you need to do is adjust "\
    ...
    def work():
        print_lang(name + ", don't take too much stress. I can list some really cool "\
    ...
    def sad1():
        print_lang('I understand. Seems like something\'s bothering you. '\
              'Could you further describe it, in short?\n')
        response = input_lang()
        if(predict(response)>=0.4):
            print_lang('It seems like though the issue might be a little '\
                  'worrisome, it might not actually be very serious. '\
                  'What are your thoughts on this?\n')
            response = input_lang()
            if(predict(response)>=0.5):
                print_lang('Looks like you agree with me. Wanna sign off?\n')
                response = input_lang()
                if(predict(response)>0.55):
                    print_lang("That's okay. It was nice talking to you. You can chat "\
    ...
    def sad2():
        print_lang('Please feel free to share your feelings ' + name +\
              ', think of me as your friend.\n')
        response = input_lang()
        if(predict(response)>=0.3):
            print_lang('I see. Among the thoughts occuring in your mind, '\
                  'which one upsets you the most?\n')
            response = input_lang()
            print_lang('Why do you think it upsets you?\n')
            response = input_lang()
            print_lang("Okay. You just identified what we call an automatic thought. "\
            ...
            print_lang('So, ' + name + ', are there signs that contrary '\
                  'could be true?\n')
            response = input_lang()
            if(predict(response)>=0.4):
                print_lang("I'm glad that you realised that the opposite could be "\
    ...
    def sad3():
        print_lang('Feel comfortable. Could you briefly explain about your '\
              'day?\n')
        response = input_lang()
        print_lang('What are the activities that make up your most of the '\
              'day?\n')
        response = input_lang()
        print_lang('It looks like you might be feeling comfortable talking '\
    ...
    def sad4():
        print_lang("My sympathies. Looks like it might be a point of concern. Don't "\
              "worry, that's what I'm here for!")
        print_lang('How are things going on with your friends?\n')
        response_friends = input_lang()
        print_lang('How is your relationship with your parents?\n')
        response_family  = input_lang()
        print_lang('How is your work or academic life going on?\n')
        response_worklife = input_lang()
    ...
    print_lang('Please tell me your name.\n')
    name = input_lang()
    try:
    ...
    print_lang("Hi " + name + "! My name's Jolly. Let's start with our session.")
    print_lang("How are you doing?\n")
    response = input_lang()
    if (predict(response) >= 0.55):
        print_lang('That is good. Are you usually this happy, or are there '\
              'some worries that you want to talk about?\n')
        response = input_lang()
        if (predict(response)>=0.7):
            print_lang('You seem to be really content. Wanna sign off?\n')
            response = input_lang()
            if(predict(response)>=0.7):
                print_lang('Ok, bye ' + name + '!')
            else:
                print_lang('Is there something bothering you? Would you '\
                      'share it with me?\n')
                response = input_lang()
                if(predict(response)>=0.7):
                    print_lang("That's okay. It was nice talking to you. You can chat "\
